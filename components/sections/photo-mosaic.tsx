...
export function PhotoMosaic({ images, altTexts, testId }: PhotoMosaicProps): React.JSX.Element {
  return (
      ...
      {images.map((src, index) => (
        <div key={src} className={`relative overflow-hidden rounded-xl2 border border-gold-300/40 ${index === 1 ? "sm:translate-y-4" : ""}`}>
          <div className="relative aspect-[4/5]">
            <Image src={src} alt={altTexts?.[index] ?? "Wedding detail image"} fill className="object-cover" />
          </div>
        </div>
      ))}
    </div>
