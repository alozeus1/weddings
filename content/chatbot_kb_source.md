---
meta:
  version: v1
  couple: "Jessica and Chibuike"
  timezone: "America/Denver"
  weddingDate: "2026-06-12"
  city: "El Paso, Texas"
  hashtag: "#JessicaAndChibuike2026"
---

# Wedding Assistant Knowledge Base

## Facts
> Keep facts short, factual, and specific. The bot must only answer from what’s written here.

### Ceremony
- Ceremony (Mass) time: 3:00 PM - 4:00 PM (MT)
- Venue: St. Patrick's Cathedral
- Address: 1118 N Mesa St, El Paso, TX 79902
- Dress code: Dress elegantly.
- Arrival: Please arrive 20-30 minutes early so everyone is seated before procession.
- Adults-only: Yes (no kids).
- Unplugged policy: Please keep phones on silent and avoid blocking aisles during Mass.

### Reception / Traditional
- Reception/Traditional time: 6:00 PM - 10:30 PM (MT)
- Venue: Tuscany Event Center
- Address: 8600 Gateway Blvd, El Paso, TX 79907
- Dress code: African Native / cultural Attire
- Open bar: Yes
- Meal selection: Meal selection is required in RSVP.
- Cultural moments: Reception includes a traditional celebration and cultural moments.

### After Party
- After party time: 10:30 PM - Midnight (MT)
- Dress code: freestyle
- Location: (TBD)

### RSVP
- RSVP deadline: 2026-05-13
- RSVP passphrase: JC2026
- Plus-ones: Check with the couple
- How to RSVP: Search your name, then enter the passphrase and complete the form.
- RSVP changes: If plans change after RSVP, contact the couple directly.
- Dietary restrictions: Please include dietary restrictions in your RSVP.

### Colors of the Day
- Sage green and dusty pink

### Registry
- Amazon: https://www.amazon.com/wedding/share/ChibuikeJessica2026
- Walmart: https://www.walmart.com/cp/wedding-registry/1229486

### Photo Uploads (Live Gallery)
- Upload page: /upload
- QR page: /qr
- Guests can upload photos on mobile.
- Upload formats: JPEG, PNG, WEBP, and HEIC are accepted up to 10 MB.
- Approved photos will appear in the Live Uploads section of the Gallery.

### Travel
- Airport: El Paso International Airport (ELP)
- Transportation: Google Maps links are available for both venues. Shuttle info is not provided.
- Parking: Parking details are not published yet; check /travel for the latest updates.
- Hotels: Hotel recommendations will be shared with guests.

---

## FAQs (Q -> A)
> Add Q/A here when you want the bot to answer very directly. Keep answers short.

- Q: What time is the ceremony?
  A: Ceremony (Mass) is 3:00 PM - 4:00 PM (MT) at St. Patrick's Cathedral.

- Q: What is the dress code for the church?
  A: Dress elegantly.

- Q: What time is the reception?
  A: Reception/Traditional is 6:00 PM - 10:30 PM (MT) at Tuscany Event Center.

- Q: What is the reception dress code?
  A: African Native / cultural Attire.

- Q: What time is the after party?
  A: The after party is 10:30 PM - Midnight (MT). Location will be shared soon.

- Q: Are kids allowed?
  A: It's an adults-only celebration (no kids).

- Q: How do I RSVP?
  A: Go to the RSVP page, search your name, enter the passphrase JC2026, and submit your response.

- Q: What is the RSVP passphrase?
  A: The RSVP passphrase is JC2026.

- Q: How do I upload photos?
  A: Visit the Upload page on your phone and upload your photos. Approved photos will show in the Live Gallery.

- Q: What airport should I fly into?
  A: Fly into El Paso International Airport (ELP).

- Q: What should I know about parking and transportation?
  A: Google Maps links are available for both venues. Shuttle info is not provided, and parking details have not been published yet.

- Q: Are there hotel recommendations?
  A: Hotel recommendations will be shared with guests.

- Q: What is the menu?
  A: Meal selection is required in RSVP, and more menu details will be shared with guests.

- Q: What meals are served?
  A: Meal selection is required in RSVP, and more menu details will be shared with guests.

- Q: Is meal selection required?
  A: Yes. Meal selection is required in RSVP.

- Q: How do I share dietary restrictions?
  A: Please include dietary restrictions in your RSVP.

- Q: Are phones okay during the ceremony?
  A: Please keep phones on silent and avoid blocking aisles during Mass.

- Q: Will there be drinks?
  A: Yes. There will be an open bar.

---

## Stories (Optional)
> Only include if you want the bot to answer story questions.

### How we met
They met online, started with friendship, and built a long-distance relationship through intentional communication and consistency.

### The proposal
The proposal happened during a vacation evening on a rooftop overlooking the city skyline.

### Note to guests
They are most excited about building a life together and choosing each other every day, and they are grateful their loved ones get to witness the beginning of that chapter.

---

## Routing Hints (Optional)
> Helps the bot suggest the right page link.

- If question mentions RSVP -> /rsvp
- If question mentions dress code or schedule -> /weekend
- If question mentions church or ceremony -> /church
- If question mentions travel/hotels/airport -> /travel
- If question mentions registry/gifts -> /registry
- If question mentions photos/upload/QR -> /upload
- If question mentions adults-only/no kids -> /faq
