...
import { listNotOnListAttendingRSVPs } from "@/lib/storage";
...
export default async function NotOnListAttendeesPage(): Promise<React.JSX.Element> {
  ...
  const guests = await listNotOnListAttendingRSVPs();

  return (
    <section className="container-shell space-y-6 py-16">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-4xl text-ink">Not-on-List Attendees</h1>
          <p className="mt-2 text-sm text-ink/70">Guests who used the open RSVP path and marked themselves as attending.</p>
        </div>
        <Link
          href="/admin/rsvps"
      ...
      <div className="overflow-x-auto rounded-2xl border border-gold-300/40 bg-white/80 shadow-card">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {guests.length === 0 ? (
              <tr className="border-t border-gold-200/40">
                <td className="px-4 py-3 text-ink/70" colSpan={4}>
                  No not-on-list attendees yet.
                </td>
              </tr>
            ) : (
              guests.map((guest) => (
